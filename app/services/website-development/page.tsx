...
export default function WebsiteDevelopment() {
  ...
  return (
    <main className="min-h-screen bg-cream text-black selection:bg-hotpink selection:text-white pb-0">
      ...
      {/* Solution Section */}
      <section className="bg-lime py-24 md:py-32 px-4 border-y-[4px] border-black shadow-[inset_0_4px_0_0_#000]">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="font-display font-black text-5xl md:text-7xl leading-[1] uppercase tracking-tighter mb-16 max-w-5xl mx-auto">
            We build websites that make your business look serious.
          </h2>
          <div className="flex flex-wrap justify-center gap-4">
            {solutions.map((item) => (
              <div key={item} className="bg-black text-white border-[3px] border-black rounded-full px-6 py-3 font-bold uppercase text-sm md:text-base hover:bg-cream hover:text-black transition-colors cursor-default">
                + {item}
              </div>
            ))}
          </div>
      ...
      <section className="py-24 md:py-32 px-4 bg-cream">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-display font-black text-4xl md:text-6xl uppercase tracking-tighter mb-16 text-center">Websites we build.</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {types.map((type, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="bg-electricblue text-white border-[3px] border-black p-6 rounded-2xl shadow-[4px_4px_0_0_#000] font-bold text-center uppercase flex items-center justify-center min-h-[120px] hover:bg-black transition-colors">
                {type}
              </motion.div>
            ))}
          </div>
